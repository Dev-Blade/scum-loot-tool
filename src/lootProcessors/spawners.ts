...
import {spawnersConfig} from './spawnersConfigs/config';
...
export const processSpawners = async () => {
  const keys = Object.keys(spawnersConfig) as spawnersConfigKey[];

  for (const key of keys) {
    const config = spawnersConfig[key];
    const {defaults, filter, pathSpawnersOverride, skipFilePatterns, exportOnlyAffectedFiles} = config;
          ...
          node.Ids.forEach((id, idIndex) => {
            ...
            if (filterKey === '*' || id.includes(filterKey)) {
              ...
              if (fv.replace !== undefined) {
                //console.log('--- replacing ' + nodes[nodeIndex].Ids[idIndex] + ' with ' + fv.replace);
    ...
    try {
      ...
      console.log('*** Processing ' + files.length + " spawners files for '" + key + "'");
      ...
      for (const file of files) {
            ...
            if (matchIndex >= 0) {
              ...
              if (!isExclude) {
                if (isArray(fixedItems)) {
                  const {result, affected} = processFixedItems(key, file, fixedItems);
                  json.FixedItems = result;
                  //console.log('fixed', file, affected);
                  if (allAffected === false) allAffected = affected;
                }

                const nodes = json.Nodes;
                if (isArray(nodes)) {
                  const {result, affected} = processNodes(key, file, nodes);
                  //console.log('nodes', file, affected);
                  json.Nodes = result;
                  if (allAffected === false) allAffected = affected;
                }
                ...
                if (isArray(items)) {
                  const {result, affected} = processItems(key, file, items);
                  //console.log('items', file, affected);
                  json.Items = result;
                  if (allAffected === false) allAffected = affected;
                }
                const m = filter[key];
                ...
                if (shouldFilterItemsByZone !== undefined && m.ShouldFilterItemsByZone !== undefined) {
        ...
        } else allAffected = false; // skip = no impact
        ...
        if (exportOnlyAffectedFiles === false || allAffected) {
