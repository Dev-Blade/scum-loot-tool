...
import {
  SpawnerFilterItemType,
  SpawnerFilterNodeType,
  ItemType,
  SpawnerNodeType,
} from "./typesSpawner";
...
export const processSpawners = async (which: spawnersConfigKey) => {
  const { defaults, filter, pathSpawnersOverride, skipFilePatterns } =
    spawnersConfig[which];
        ...
        node.Ids.forEach((id, idIndex) => {
          ...
          if (filterKey === "*" || id.includes(filterKey)) {
            ...
            if (fv.replace !== undefined) {
              ...
              nodes[nodeIndex].Ids[idIndex] = nodes[nodeIndex].Ids[
                idIndex
              ].replace(fv.replace[0], fv.replace[1]);
            }
            if (fv.remove === true) {
              // console.log(
              //   "--- removing " +
            ...
            if (fv.listAdd !== undefined) {
              let toPush: string[] = [];
              if (isArray(fv.listAdd)) toPush = [...fv.listAdd];
              else toPush = [fv.listAdd];
              for (const tp of toPush) {
                if (!node.Ids.includes(tp)) nodes[nodeIndex].Ids.push(tp);
              }
            }
          }
          return true;
  ...
  const processItems = (
    ...
    let list = items.map((item) => {
      const index = filterKeys.findIndex(
        (element) => element === "*" || item.Id?.includes(element)
      );
  ...
  const processFixedItems = (
    ...
    if (filterKeys.length === 1 && filterKeys[0] === "*") {
      ...
      let list = items.map((item) => {
        const index = filterKeys.findIndex(
          (element) => element === "*" || item.includes(element)
        );
  ...
  try {
    ...
    console.log(
      "*** Processing " + files.length + " spawners files for '" + which + "'"
    );
    ...
    for (const file of files) {
      ...
      if (!skip) {
        ...
        if (!isNaN(probability)) {
          probability = Math.round(
            probability * defaults.defaultProbabilityMultiplier
          );
        ...
        if (initialDamage === 0)
          initialDamage =
            Math.random() *
            defaults.defaultInitialDamageRandomMaxMultiplierIfZero;

        if (!isNaN(initialDamage)) {
          initialDamage = Math.round(
            initialDamage * defaults.defaultInitialDamageMultiplier
          );
          ...
          if (
            defaults.defaultInitialMinDamage > 0 &&
            initialDamage < defaults.defaultInitialMinDamage
          )
            initialDamage = defaults.defaultInitialMinDamage;
        ...
        if (!isNaN(randomDamage)) {
          randomDamage = Math.round(
            randomDamage * defaults.defaultRandomDamageMultiplier
          );
          ...
          if (
            defaults.defaultRandomMinDamage > 0 &&
            randomDamage < defaults.defaultRandomMinDamage
          )
            randomDamage = defaults.defaultRandomMinDamage;
        ...
        if (!isNaN(quantityMin)) {
          quantityMin = Math.round(
            quantityMin * defaults.defaultQuantityMinMultiplier
          );
        ...
        if (!isNaN(quantityMax)) {
          quantityMax = Math.round(
            quantityMax * defaults.defaultQuantityMaxMultiplier
          );
        ...
        if (!isNaN(initialUsage)) {
          initialUsage = Math.round(
            initialUsage * defaults.defaultInitialUsageMultiplier
          );
          json.InitialUsage = initialUsage;
        }
        let randomUsage = parseInt(json.RandomUsage);
        if (!isNaN(randomUsage)) {
          randomUsage = Math.round(
            randomUsage * defaults.defaultRandomUsageMultiplier
          );
          ...
          if (matchIndex >= 0) {
            ...
            if (!isExclude) {
              ...
              if (
                ...
                initialDamage = json.InitialDamage = Math.round(
                  initialDamage * m.InitialDamageMultiplier
                );
              }
              ...
              if (
                ...
                randomDamage = json.RandomDamage = Math.round(
                  randomDamage * m.RandomDamageMultiplier
                );
              }
              ...
              if (
                ...
                initialUsage = json.InitialUsage = Math.round(
                  initialUsage * m.InitialUsageMultiplier
                );
              }
              ...
              if (
                ...
                randomUsage = json.RandomUsage = Math.round(
                  randomUsage * m.RandomUsageMultiplier
                );
              }
              ...
              if (
                ...
                quantityMin = json.QuantityMin = Math.round(
                  quantityMin * m.QuantityMinMultiplier
                );
              }
              ...
              if (
                ...
                quantityMax = json.QuantityMax = Math.round(
                  quantityMax * m.QuantityMaxMultiplier
                );
              }
              ...
              const shouldFilterItemsByZone = json.ShouldFilterItemsByZone;
              if (
                shouldFilterItemsByZone !== undefined &&
                m.ShouldFilterItemsByZone !== undefined
              ) {
                json.ShouldFilterItemsByZone = m.ShouldFilterItemsByZone;
              }
      ...
      if (e === false)
        throw new Error(
          "Could not create target directory " + pathSpawnersOverride
        );
    ...
    console.log(err);
    console.log(
      "Error processing file " + fullName + " - file#" + fileCount + 1
    );
  }
};
